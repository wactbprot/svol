...
for(l.sitz in 1:length(sn.sitz)){
    for(l.teller in 1:length(sn.teller)){
        ...
        ## der wird definiert wo Abstand zur sitzgeraden > 50µm wird

        j <- which(r.sitz < brs & (m.sitz*r.sitz + c.sitz - z.sitz) < -0.1)
        ...
        plot(r.sitz,z.sitz
           ...
           , xlab = xlab
           , ylab = ylab
           , main=paste("scan:", sn.sitz[l.sitz]))
        points(r.sitz[i], z.sitz[i], col=2)

        grid(col=1)
        abline(lm.sitz <- lm(z.sitz[i] ~ r.sitz[i]), col=3)
        ...
        plot(r.teller, z.teller
           ...
           , xlab = xlab
           , ylab = ylab
           , main=paste("scan:", sn.teller[l.teller])
             )
        grid(col=1)
